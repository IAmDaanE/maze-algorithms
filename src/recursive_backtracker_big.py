...
visited_cells = set()
current_snake = []
current_cell = [random.randint(0,99),random.randint(0,99)]
...
while running:
    ...
    if not data_dumped and complete:
        ...
        with open("../mazes/big_maze.json", "w") as f:
    ...
    for row in range(100):
        for column in range(100):
            ...
            if walls_left[row][column]:
                pygame.draw.rect(screen, maze_color, (x,y,1,7))
            if walls_top[row][column]:
                pygame.draw.rect(screen, maze_color, (x,y,7,1))
